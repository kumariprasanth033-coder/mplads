  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`w-full px-3 py-2 border rounded-xl text-xs font-semibold flex items-center justify-between gap-2 shadow-2xs transition-all ${
          isDisabled
            ...
            : 'bg-slate-50 hover:bg-white text-slate-800 border-slate-200 hover:border-slate-300 cursor-pointer focus:outline-hidden focus:ring-2 focus:ring-blue-800'
        }`}
      >
      ...
      {isOpen && !isDisabled && (
        <div
          role="listbox"
          className="fixed sm:absolute z-50 left-4 right-4 sm:left-0 sm:right-auto sm:w-84 mt-1 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden text-xs flex flex-col animate-in fade-in zoom-in-95 duration-150 max-h-[380px]"
          style={{
        ...
        >
          ...
          <div className="p-2.5 border-b border-slate-100 bg-slate-50/80 sticky top-0 z-10 flex items-center gap-2">
            ...
            <input
              ...
              className="w-full py-1 text-xs bg-transparent border-none text-slate-900 placeholder:text-slate-400 focus:outline-hidden"
            />
            {searchQuery && (
              <button
                type="button"
