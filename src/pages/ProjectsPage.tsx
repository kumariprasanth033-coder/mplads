  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        {/* Search & Filter Bar */}
        <div className="bg-slate-800/90 rounded-2xl border border-slate-700 shadow-xs p-4 mb-6">
          <form onSubmit={handleSearchSubmit} className="flex flex-col md:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 text-xs bg-slate-900 border border-slate-700 rounded-xl focus:outline-hidden focus:ring-2 focus:ring-blue-500 text-slate-100 placeholder-slate-400"
              />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <select
                ...
                className="px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl focus:outline-hidden text-slate-200 font-medium max-w-[140px]"
              >
              ...
              <select
                ...
                disabled={!state}
                className="px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl focus:outline-hidden text-slate-200 font-medium disabled:bg-slate-800 disabled:text-slate-500 max-w-[140px]"
              >
              ...
              <select
                ...
                className="px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl focus:outline-hidden text-slate-200 font-medium"
              >
              ...
              <button
                type="submit"
                className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold transition-colors cursor-pointer"
              >
                Apply
              </button>
              ...
              {(query || sector || status || state || district || risk) && (
                <button
                  ...
                  className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl font-semibold transition-colors cursor-pointer text-xs border border-slate-700"
                >
                  Reset
                </button>
              )}
          ...
          {(state || district || status || sector || risk || query) && (
            <div className="mt-3 pt-3 border-t border-slate-700/60 flex flex-wrap items-center gap-2 text-xs">
              ...
              {state && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-blue-900/40 text-blue-300 border border-blue-700/50 font-semibold">
                  <span>State: {state}</span>
                  <button type="button" onClick={() => { setState(''); setDistrict(''); setPage(1); }} className="hover:text-red-400">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
              {district && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-purple-900/40 text-purple-300 border border-purple-700/50 font-semibold">
                  <span>District: {district}</span>
                  <button type="button" onClick={() => { setDistrict(''); setPage(1); }} className="hover:text-red-400">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
              {status && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-amber-900/40 text-amber-300 border border-amber-700/50 font-semibold">
                  <span>Status: {status}</span>
                  <button type="button" onClick={() => { setStatus(''); setPage(1); }} className="hover:text-red-400">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
              {sector && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-emerald-900/40 text-emerald-300 border border-emerald-700/50 font-semibold">
                  <span>Sector: {sector}</span>
                  <button type="button" onClick={() => { setSector(''); setPage(1); }} className="hover:text-red-400">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
              {risk && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-rose-900/40 text-rose-300 border border-rose-700/50 font-semibold">
                  <span>Risk: {risk}</span>
                  <button type="button" onClick={() => { setRisk(''); setPage(1); }} className="hover:text-red-400">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
              {query && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-slate-800 text-slate-200 border border-slate-700 font-semibold">
                  <span>Query: &ldquo;{query}&rdquo;</span>
                  <button type="button" onClick={() => { setQuery(''); setPage(1); }} className="hover:text-red-400">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
        ...
        {isLoading ? (
          ...
          <div className="py-20 text-center bg-slate-800/90 rounded-2xl border border-slate-700 p-8">
            <FolderGit2 className="w-12 h-12 text-slate-500 mx-auto mb-3" />
            <h3 className="font-bold text-slate-200 text-base">No Projects Found</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map(proj => (
              <div
                ...
                className="bg-slate-800/90 rounded-2xl border border-slate-700 hover:border-blue-400 hover:shadow-lg transition-all cursor-pointer flex flex-col justify-between overflow-hidden group"
              >
                <div>
                  {/* Photo header */}
                  ...
                  <div className="p-5">
                    <div className="flex items-center gap-2">
                      <span className="text-[11px] font-bold text-blue-300 bg-blue-900/50 px-2 py-0.5 rounded border border-blue-700/50">
                        {proj.category}
                      </span>
                      {proj.riskCategory === 'HIGH' && (
                        <span className="text-[10px] font-bold text-rose-300 bg-rose-900/50 px-2 py-0.5 rounded border border-rose-700/50 flex items-center gap-1">
                    ...
                    <h3 className="font-bold text-base text-slate-100 group-hover:text-blue-400 mt-2 line-clamp-2 leading-snug">
                      {proj.title}
                    </h3>
                ...
                <div className="p-5 pt-0">
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-700/60 flex items-center justify-between text-xs font-semibold text-blue-400">
